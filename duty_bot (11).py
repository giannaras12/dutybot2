...
async def end_duty_session(user, auto=False, reason=None):
    ...
    try:
        ...
        dm_embed.add_field(name="Points Earned", value=awarded_points, inline=False)
        dm_embed.add_field(name="Total Points", value=points[user_id_str], inline=False)
...
async def schedule_reminder(user):
    ...
    while user.id in ACTIVE_DUTIES:
        try:
            ...
            remaining_time = MAX_DUTY_DURATION - current_duration
            if remaining_time.total_seconds() > 0:
                embed.add_field(name="Auto-End In", value=str(remaining_time)[:-7], inline=False)
